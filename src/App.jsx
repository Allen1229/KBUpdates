...
export default function App() {
  ...
  // 判斷輸入是否為單一網址（無其他文字），並嘗試提取 no= 參數
  const parseSingleUrl = (text) => {
    ...
    try {
      ...
      return { url: match[1], noParam: noParam || '' };
    } catch {
      return null;
    }
  };

  // 解析輸入文字中是否有分別指定歷程/說明頁的網址
  const parseMultipleUrls = (text) => {
    const lines = text.trim().split('\n').map(l => l.trim()).filter(Boolean);
    let historyUrl = '', manualUrl = '', historyText = '', manualText = '';
    let hasSpecific = false;
    for (const line of lines) {
      const historyMatch = line.match(/歷程[\s:：]*(https?:\/\/[^\s]+)/i);
      const manualMatch = line.match(/說明(?:頁)?[\s:：]*(https?:\/\/[^\s]+)/i);
      if (historyMatch) {
        historyUrl = historyMatch[1];
        try { historyText = new URL(historyMatch[1]).searchParams.get('no') || ''; } catch {}
        hasSpecific = true;
      }
      if (manualMatch) {
        manualUrl = manualMatch[1];
        try { manualText = new URL(manualMatch[1]).searchParams.get('no') || ''; } catch {}
        hasSpecific = true;
      }
    }
    return hasSpecific ? { historyUrl, historyText, manualUrl, manualText } : null;
  };
...
4. 若使用者有分別指定「歷程」和「說明頁」的網址，請分別填入對應欄位。`;
  ...
  const saveToGoogleSheet = async () => {
    ...
    // 檢查備註是否包含 SSS，用於決定是否標記粉紅色字體
    const hasSss = activeTab === 'game' && /SSS/i.test(extractedGame.notes);
